...
from CamionStockCola import CamionEspera
from Camion import Camion
from Pala import Pala
from StockPileCola import StockPileQueue
...
class OPMCTSystem(CoupledDEVS):
	def __init__(self,cQ,pQ,sQ,palaToStock,QinS):
		...
		if QinS:
			...
			for i in range(cQ):
				name = "camion_"+str(i)
				self.camiones.append(self.addSubModel(CamionEspera(sQ,pQ,name,dispatcher)))

			#Creación de StockPiles
			self.stockPiles = []
			for i in range(sQ):
				self.stockPiles.append(self.addSubModel(StockPileQueue("stock_"+str(i+1),cQ)))
